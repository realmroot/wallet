...
CREATE TABLE agent_grant_with_soft_delete (
  id TEXT PRIMARY KEY,
...
INSERT INTO agent_grant_with_soft_delete (
  ...
  period_started_at, expires_at, deleted_at, created_at, updated_at, paused_at,
...
CREATE UNIQUE INDEX agent_grant_active_identity_idx
  ON agent_grant_with_soft_delete(user_id, agent_issuer, agent_subject, mode)
  WHERE deleted_at IS NULL;

CREATE TABLE budget_request_with_soft_delete (
  id TEXT PRIMARY KEY,
  ...
  grant_id TEXT REFERENCES agent_grant_with_soft_delete(id),
...
INSERT INTO budget_request_with_soft_delete (
  id, owner_issuer, owner_subject, agent_issuer, agent_subject, status,
  approval_token_hash, grant_id, expires_at, decided_at, created_at, updated_at,
  mode
)
SELECT
  id, owner_issuer, owner_subject, agent_issuer, agent_subject, status,
...
CREATE TABLE payment_with_soft_delete (
  id TEXT PRIMARY KEY,
  user_id TEXT NOT NULL REFERENCES wallet_user(id),
  grant_id TEXT NOT NULL REFERENCES agent_grant_with_soft_delete(id),
...
INSERT INTO payment_with_soft_delete (
...
ALTER TABLE agent_grant_with_soft_delete RENAME TO agent_grant;
ALTER TABLE budget_request_with_soft_delete RENAME TO budget_request;
ALTER TABLE payment_with_soft_delete RENAME TO payment;
